...
def run_bollinger_backtest(
    ...
    data["Upper_Band"] = data["Middle_Band"] + (num_std * data["Band_Std"])
    data["Lower_Band"] = data["Middle_Band"] - (num_std * data["Band_Std"])
    ...
    data.loc[
        (data["Close"] < data["Lower_Band"]) & (data["Market_Regime"] == 1),
        "Signal"
    ] = 1
    ...
    data.loc[
        (data["Close"] > data["Middle_Band"]) | (data["Market_Regime"] == 0),
        "Signal"
    ] = 0
    ...
    buy_hold_metrics = calculate_performance_metrics(
        data, "Daily_Return", "Buy_Hold_Growth", "Buy_Hold_Drawdown", initial_capital
    )

    strategy_metrics = calculate_performance_metrics(
        data, "Strategy_Return_Final", "Strategy_Growth", "Strategy_Drawdown", initial_capital
    )
    ...
    summary = pd.DataFrame([
        {
            ...
            "Ticker": ticker,
            "Bollinger Window": np.nan,
            "Bollinger Std": np.nan,
