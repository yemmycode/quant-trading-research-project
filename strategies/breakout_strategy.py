...
def run_breakout_backtest(
    ...
    data["Rolling_High"] = data["Close"].rolling(window=breakout_window).max().shift(1)
    data["Rolling_Low"] = data["Close"].rolling(window=exit_window).min().shift(1)
    ...
    data.loc[
        (data["Close"] > data["Rolling_High"]) & (data["Market_Regime"] == 1),
        "Signal"
    ] = 1
    ...
    data.loc[
        (data["Close"] < data["Rolling_Low"]) | (data["Market_Regime"] == 0),
        "Signal"
    ] = 0
